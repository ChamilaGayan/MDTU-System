...
if(isset($_SESSION["username"]))
{
	...
	$sql_getData = "SELECT * from user,trainee,request_courses,completed_course,institute WHERE user.user_name=trainee.t_id AND user.user_name=request_courses.t_id AND user.user_name=institute.t_id AND user.user_name=completed_course.t_id AND user.user_name='$uname'";
	/* echo $sql_getData; */
...
<!-----------------------------------------------------------------request courses table----------------------------------------------------------->		
						...
						<label for="staticEmail" class="col-sm-5 col-form-label" style="text-align:left;"><b>03.ඔබ ඉල්ලුම් කරන පාඨමාලා (විෂය පථයට අදාළව)</b></label></br>
							...
							?>
								...
								<?php
										//$sql_accna="SELECT * FROM courses,request_courses where courses.course_code=request_courses.course_code1";
										$sql_accna="SELECT course_code1 FROM request_courses where request_courses.t_id='$tid'";
										$accna_info=$conn->query($sql_accna)or die(mysqli_error($conn));
										$numrows_accna=mysqli_num_rows($accna_info);
								?>
								
										<select  name="request[]"  disabled   class="form-control" required>
								...
								<?php 
									if($numrows_accna > 0)
									{
										...
										$ccid=$row_accna['course_code'];
										
										$sql_accna1="SELECT course_name FROM courses where course_code='$ccid'";
										$accna_info1=$conn->query($sql_accna1)or die(mysqli_error($conn));
										$numrows_accna1=mysqli_num_rows($accna_info1);
										
										$ccname=$row_accna['course_name'];
										
										$sql_accna2="SELECT course_name FROM courses where course_code='$ccname'";
										$accna_info2=$conn->query($sql_accna2)or die(mysqli_error($conn));
										$numrows_accna2=mysqli_num_rows($accna_info2);
				...
				<div class="row">
					<div class="col-md-4 mb-3">
						<label for="staticEmail"  style="text-align:left;">පාඨමාලාවේ නම </label>
							...
							<?php
								$sql_accna="SELECT * FROM courses,completed_course where courses.course_code=completed_course.course_code";
								$accna_info=$conn->query($sql_accna)or die(mysqli_error($conn));
								$numrows_accna=mysqli_num_rows($accna_info);
	...
	<div class="row">
		<div class="col-md-4 mb-3">
			<b>
				<?php 
				//$conn=mysqli_connect('localhost','root','','mydb');
				$conn->set_charset("utf8"); $sql_accna="SELECT * FROM courses,completed_course where courses.course_code=completed_course.course_code";
