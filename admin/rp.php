...
<!--form-->				 
<form action="rp.php" method="POST" autocomplete="off">
...
if(isset($_POST["find"]))
{
	$course=$_POST["search"];
  $dt=$_POST["dt"];
	$sql="SELECT * FROM resourses,rcourses,user WHERE resourses.r_id=rcourses.r_id AND user.user_name=resourses.r_id AND course_code='$course' AND extract(year from resourses.apply_date) = $dt";
	$i = 0;
  ...
  {
    echo "<td>" .++$i. "</td><td>" . $row["rname"]. "</td><td>" . $row["email"]."</td><td>" . $row["position"]."</td><td>" . $row["office_address"]."</td> <td>" . $row["apply_date"]. "</td> </tr>";
  }
